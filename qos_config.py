...
class queue_set_config(object):
    def __init__(self, controller_ip, controller_port, queue_config, max_R_common,ovsdb_addr_port):
        self.controller_ip=controller_ip
        self.controller_port=controller_port
        self.switchdpid_port_ip={}
        self.queue_config=queue_config
        self.max_R_common=max_R_common
        self.ovsdb_addr_port=ovsdb_addr_port
        self.conn = http.client.HTTPConnection("{}:{}".format(self.controller_ip, self.controller_port))
    ...
    def put_ovsdb(self):
        for k,v in self.switchdpid_port_ip.items():
            BODY="\"tcp:{}:{}\"".format(v['switch_ip'],self.ovsdb_addr_port)
    ...
    def post_queue(self,p_name,xdpid):
            ...
            r = requests.post('http://{}:{}/qos/queue/{}'.format(self.controller_ip,self.controller_port,xdpid, data=(BODY))
            print(r.status_code)
...
if __name__ == '__main__':
       with open('_qos.json') as i:
                config = json.load(i)
       config1=queue_set_config(config["controller_ip"],config["controller_port"],config["queue_config"], config["max_R_common"],config["ovsdb_addr_port"])
       config1.topo_info()
